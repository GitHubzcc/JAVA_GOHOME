...
public class HomeMethods extends Thread {
	...
	private void SubmitOrder() {
		String username = home_page.passengerList.getModel().getElementAt(0).toString();
		JSONObject userObj = (JSONObject) home_page.userMap.get(username);
		if (obj2==null) {
			obj2 = (JSONObject) home_page.datalist.get(home_page.trainTable.getSelectedRow());
		}
		DefaultListModel<Object> model = (DefaultListModel<Object>)home_page.seatTypeList.getModel();
		...
		try {
			...
			parames.put("train_date", home_page.dateField.getText());
			parames.put("back_train_date", home_page.dateField.getText());
	...
	private void getQueueCount() {
		...
		try {
			parames4.put("train_date", home_page.format2.parse(home_page.dateField.getText())
					+ "");
